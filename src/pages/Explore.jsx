...
const Explore = () => {
    ...
    const { data: trendingPosts } = useQuery({
        ...
        queryFn: async () => {
            ...
            return posts.sort((a, b) => b.likes.length - a.likes.length).slice(0, 5);
        },
        staleTime: 1000 * 60 * 5,
    });
                    ...
                    <Tooltip
                        label={dateRange[0] && typeof dateRange[0] === 'object' ?
                            (dateRange[0].toDateString() === dateRange[1]?.toDateString() ?
                                `Filter by date: ${dateRange[0].toLocaleDateString()} (Press Ctrl+D to focus, Esc to clear)` :
                                `Filter by date: ${dateRange[0].toLocaleDateString()} - ${dateRange[1]?.toLocaleDateString()} (Press Ctrl+D to focus, Esc to clear)`)
